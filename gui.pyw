...
class MainWindow:
    def __init__(self):
        ...
        presets_label.grid(row=1, column=1)
        ...
        presets_section_frame.grid(row=3, column=1, columnspan=3, padx=15, pady=15)
        ...
        move_speed_frame.grid(row=4, column=1, padx=15, pady=15)
        ...
        pt_frame.grid(row=4, column=2, padx=15, pady=15)

        pt_options_frame = Frame(main_frame)

        stop_move = Checkbutton(pt_options_frame, onvalue=True, offvalue=False, variable=stop_move_on_release)
        stop_move.grid(row=2, column=1)

        stop_move_label = Label(pt_options_frame, text="Stop moving on release")
        stop_move_label.grid(row=2, column=2)

        pt_options_frame.grid(row=5, column=1, columnspan=3, padx=15, pady=15)
        ...
        zoom_frame.grid(row=6, column=1, padx=15, pady=15)

        focus_frame = Frame(main_frame)

        stop_focus_on_release = tk.BooleanVar(focus_frame, True)

        focus_speed_label = Label(focus_frame, text="Focus:")
        focus_speed_label.grid(row=1, column=1)

        focus_speed_val = tk.StringVar()
        focus_speed_val.set("0")

        focus_speed_info = Label(focus_frame, textvariable=focus_speed_val)
        focus_speed_info.grid(row=2, column=1)

        def processFocus(speed):
            if int(focus_speed_val.get()) != speed:
                focus_speed_val.set(speed)
                if speed > 0:
                    self.cam.focusNear(speed)
                elif speed < 0:
                    self.cam.focusFar(speed * -1)
                else:
                    self.cam.focusStop()

        def resetFocus(slider):
            if stop_focus_on_release.get():
                slider.set("0")

        focus_speed_slider = Scale(focus_frame, orient=tk.VERTICAL, from_=7, to=-7, value=0, command=lambda val: processFocus(int(float(val))))
        focus_speed_slider.bind("<ButtonRelease-1>", lambda _: resetFocus(focus_speed_slider))
        focus_speed_slider.grid(row=6, column=1)

        focus_frame.grid(row=6, column=2, padx=15, pady=15)

        focus_zoom_options_frame = Frame(main_frame)

        stop_zoom = Checkbutton(focus_zoom_options_frame, onvalue=True, offvalue=False, variable=stop_zoom_on_release)
        stop_zoom.grid(row=1, column=1)

        stop_zoom_label = Label(focus_zoom_options_frame, text="Stop zooming on release")
        stop_zoom_label.grid(row=1, column=2)

        stop_focus = Checkbutton(focus_zoom_options_frame, onvalue=True, offvalue=False, variable=stop_focus_on_release)
        stop_focus.grid(row=2, column=1)

        stop_focus_label = Label(focus_zoom_options_frame, text="Stop focusing on release")
        stop_focus_label.grid(row=2, column=2)

        focus_zoom_options_frame.grid(row=7, column=1, columnspan=3, padx=15, pady=15)
